...
public class DescriptionView extends ViewPart {
	...
	public DescriptionView() {
		ProjectManager.getInstance().setDescriptionView(this);
	}
...
//		this.getSite().getWorkbenchWindow().getWorkbench().getImportWizardRegistry()
		
		
		
		viewer = new Text(parent, SWT.BORDER | SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL);
		GridData gridData = new GridData(GridData.FILL, GridData.FILL, true, true);
		gridData.widthHint = 400;
		viewer.setLayoutData(gridData);
		viewer.setEditable(false);
		viewer.setBackground(viewer.getDisplay().getSystemColor(SWT.COLOR_INFO_BACKGROUND));
		viewer.setForeground(viewer.getDisplay().getSystemColor(SWT.COLOR_INFO_FOREGROUND));
		viewer.setText("");
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				DescriptionView.this.fillContextMenu(manager);
			}
		});
