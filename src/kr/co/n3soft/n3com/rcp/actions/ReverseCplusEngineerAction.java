...
public class ReverseCplusEngineerAction extends Action {
	
	IWorkbenchWindow workbenchWindow;
	public static   int ID=0;
	IEditorInput input = null;
	public ReverseCplusEngineerAction(IWorkbenchWindow window) {
		setId("ReverseCplusEngineerAction");
		this.setText("C++");//PKY 08072201 S 도구 툴바 통합
		workbenchWindow = window;
		setImageDescriptor(ImageDescriptor.createFromImage(ProjectManager.getInstance().getImage(313)));//PKY 08070101 S 팝업 메뉴 이미지 삽입
	}
...
//			DirectoryDialog fsd = new DirectoryDialog(workbenchWindow.getShell(),SWT.OPEN);
			...
			fsd.setFilterExtensions(new String[] {"*.h","*.*"});
