...
public class InterfaceFigure extends UMLFigure implements HandleBounds {
    private IFigure pane;
    private TextFlow textFlow;
    private boolean isMode = true;
  //PKY 08090401 S 인터페이스 Multiplicity 표시 못하는문제
    String multi = "";
    public void setMultiplicity(String p){
	this.multi = p;
    }
    //PKY 08090401 E 인터페이스 Multiplicity 표시 못하는문제
    public InterfaceFigure() {
        //	setBorder(new BoundaryBorder());
    	setBorder(new UMLModelBorder());//PKY 08070901 S 모델 컬러 변경 //PKY 08080501 S 모델 그림자 넣기 작업
        ...
        if (this.isMode) {
          ...
          //PKY 08090401 S 인터페이스 Multiplicity 표시 못하는문제
            if(this.multi!=null && this.multi.trim().length()>0){
            	int index = this.multi.length()+2;
            	graphics.drawString("{"+this.multi+"}",r.x+r.width-7*index, r.y +2);//PKY 08080501 S Multiplicity 입력시 짤려서 들어가는 문제 수정
            }
          //PKY 08090401 E 인터페이스 Multiplicity 표시 못하는문제
        }
        else {
