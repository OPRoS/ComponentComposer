...
public class FinalActionFigure extends UMLFigure implements HandleBounds {
    private IFigure pane;
    private TextFlow textFlow;
    private int type = 0;//PKY 08072401 S Action Action 저장 불러오기시 그림이 안되는문제
    private FinalActiionModel model = null;//PKY 08072401 S Action Action 저장 불러오기시 그림이 안되는문제

    public FinalActionFigure() {
        setBorder(new FinalActionBorder());
      ...
      //PKY 08072401 S Action Action 저장 불러오기시 그림이 안되는문제
        if(model!=null){
        	if(type==2){
        		 ...
        		 int y = 0;
        		 int x = model.getLocation().x;
        		 if(model.getLocation().x>=0)
        			 x= model.getLocation().x + model.getSize().width -10;
        		 else
        			 x= model.getLocation().x + model.getSize().width -10;
        		 if(model.getLocation().y>=0)
        			 y= model.getLocation().y + model.getSize().height -10;
        		 else 
        			 y= model.getLocation().y + model.getSize().height -10;
//        		 int y = r.y + r.height;
//        	        int x = r.x + r.width;
                ...
                int y1 = r.y;
                if(model.getStereotype()!=null&&!model.getStereotype().equals(""))
                graphics.drawText("(:)", x1, y1+35);
                else
                    graphics.drawText("(:)", x1, y1+15);
        	}
	...
	public FinalActiionModel getModel() {
		return model;
	}

	public void setModel(FinalActiionModel model) {
		this.model = model;
	}
}
