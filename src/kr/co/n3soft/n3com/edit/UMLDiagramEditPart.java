...
public class UMLDiagramEditPart extends UMLContainerEditPart implements LayerConstants {
...
//        BoundaryFigure f = new BoundaryFigure();
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (UMLDiagramModel.ID_ROUTER.equals(evt.getPropertyName()))
            refreshVisuals();
        else
    ...
    protected void refreshVisuals() {
        if (getUMLModel() instanceof N3EditorDiagramModel) {
        	N3EditorDiagramModel actorModel = (N3EditorDiagramModel)getUMLModel();
...
//        cLayer.setConnectionRouter(new ManhattanConnectionRouter());
        ...
        if (getUMLDiagramModel().getConnectionRouter().equals(UMLDiagramModel.ROUTER_MANUAL)) {
            AutomaticRouter router = new FanRouter();
            router.setNextRouter(new BendpointConnectionRouter());
            cLayer.setConnectionRouter(router);
        } else if (getUMLDiagramModel().getConnectionRouter().equals(UMLDiagramModel.ROUTER_MANHATTAN))
            cLayer.setConnectionRouter(new ManhattanConnectionRouter());
        else
            cLayer.setConnectionRouter(new ShortestPathConnectionRouter(getFigure()));
        Animation.run(400);
