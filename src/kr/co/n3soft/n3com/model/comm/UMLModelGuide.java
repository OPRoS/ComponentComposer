...
public class UMLModelGuide implements Serializable {
    ...
    public UMLModelGuide() {
        // empty constructor
    }

    /**
     * Constructor
     * @param	isHorizontal	<code>true</code> if the guide is horizontal (i.e., placed on a vertical ruler)
     */
    public UMLModelGuide(boolean isHorizontal) {
     ...
     */
    public void attachPart(UMLModel part, int alignment) {
        if (getMap().containsKey(part) && getAlignment(part) == alignment)
            return;
        getMap().put(part, new Integer(alignment));
        UMLModelGuide parent = isHorizontal() ? part.getHorizontalGuide() : part.getVerticalGuide();
     ...
     */
    public void detachPart(UMLModel part) {
     ...
     * This information is used by {@link org.eclipse.gef.examples.logicdesigner.edit.LogicXYLayoutEditPolicy
     * LogicXYLayoutEditPolicy} to determine whether to attach or detach a part from a guide during resize operations.
     ...
     * EditPart, Object)
     */
    public int getAlignment(UMLModel part) {
     ...
     * @param	isHorizontal	<code>true</code> if this guide is to be placed on a vertical ruler
     */
